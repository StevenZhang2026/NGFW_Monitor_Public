...
- v2.2 之前的 ACC 数据来自 Log Query API，有 `nlogs` 上限，流量大的小时被截断得越狠（实测某小时库里 81MB vs 实际 681MB）。跨 v2.2 切换点做环比、趋势斜率和容量预测时这批历史数据不可比
- `cpu_usage` 曾采到 112.3%（`%Cpu(s)` 不应超过 100），解析规则待查，影响 CPU 阈值告警和容量预测
