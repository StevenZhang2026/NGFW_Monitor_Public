...
1. 告警规则端到端验证（配置阈值规则 → 触发 → 收到通知）
2. 接入更多设备（PA-5500/PA-7000 系列）验证兼容性
3. 性能调优（采集间隔精细控制、worker 并发优化）
4. Panorama 设备自动发现
...
- `backend/app/collectors/` — 采集器插件（panos_api, panos_ssh, panorama, file_upload）
- `backend/app/tasks/collect.py` — 核心采集调度（per-device 批量采集，连接复用）
- `backend/app/tasks/alert.py` — 告警评估任务
- `backend/app/metrics/builtin.yaml` — 内置指标定义（10 个）
...
- `frontend/src/pages/` — 前端页面（Dashboard, Devices, Metrics, Alerts, Settings, Upload）
